...
def detect_objects(imageToDetectOn, imageToDrawOn, vision_image, hsv_filter, maxThreshold, minThreshold, minArea,
                   maxArea, name, rgb_Color, threshold, minPoints, maxPoints, arenaCorners):
        ...
        if maxArea > area > minArea:
            ...
            if minPoints <= len(approx) <= maxPoints:
                ...
                x_cart, y_cart = ComputerVision.ImageProcessor.convert_to_cartesian((center_x, center_y),
                                                                                    arenaCorners[0], arenaCorners[1],
                                                                                    arenaCorners[3], arenaCorners[2])
                x_cart = round(x_cart, 2)
                y_cart = round(y_cart, 2)
                cv.putText(imageToDrawOn, f"{name}: {len(approx)}", (x + w + 20, y - 5), cv.FONT_HERSHEY_COMPLEX, 0.7,
                           rgb_Color, 1)
                cv.putText(imageToDrawOn, f"Points: {len(approx)}", (x + w + 20, y + 20), cv.FONT_HERSHEY_COMPLEX, 0.7,
                           rgb_Color, 1)
                cv.putText(imageToDrawOn, f"Area: {int(area)}", (x + w + 20, y + 45), cv.FONT_HERSHEY_COMPLEX, 0.7,
                           rgb_Color, 1)
                cv.putText(imageToDrawOn, f"(x,y): ({x_cart},{y_cart})", (x + w + 20, y + 70), cv.FONT_HERSHEY_COMPLEX,
                           0.7, rgb_Color, 1)
    ...
    for cnt in contours:
        ...
        if maxArea > area > minArea:
            ...
            if 6 <= len(approx) <= 10:
                ...
                cv.putText(imageToDrawOn, "Balls: " + str(len(approx)), (x + w + 20, y + -5), cv.FONT_HERSHEY_COMPLEX,
                           .7, (0, 0, 255), 2)
                cv.putText(imageToDrawOn, "Points: " + str(len(approx)), (x + w + 20, y + 20), cv.FONT_HERSHEY_COMPLEX,
                           .7, (0, 0, 255), 2)
                cv.putText(imageToDrawOn, "Area: " + str(int(area)), (x + w + 20, y + 45), cv.FONT_HERSHEY_COMPLEX, .7,
                           (0, 0, 255), 2)
                for point in approx:
...
def findArena(imageToDetectOn, imageToDrawOn):
    ...
    for cnt in contours:
        ...
        if maxArea > area > minArea:
            ...
            cv.putText(imageToDrawOn, "Points: " + str(len(approx)), (x + w + 20, y + 20), cv.FONT_HERSHEY_COMPLEX, .7,
                       (0, 0, 0), 2)
            cv.putText(imageToDrawOn, "Area: " + str(int(area)), (x + w + 20, y + 45), cv.FONT_HERSHEY_COMPLEX, .7,
                       (0, 0, 0), 2)
            for point in approx:
...
def findObstacles(screenshot, output_image):
    ...
    for cnt in contours:
        ...
        if maxArea > area > minArea:
            ...
            cv.putText(output_image, "Points: " + str(len(approx)), (x + w + 20, y + 20), cv.FONT_HERSHEY_COMPLEX, .7,
                       (0, 255, 0), 2)
            cv.putText(output_image, "Area: " + str(int(area)), (x + w + 20, y + 45), cv.FONT_HERSHEY_COMPLEX, .7,
                       (0, 255, 0), 2)
            for point in approx:
...
async def main(mode):
    ...
    while (True):
        try:
            ...
            # cross
            edged, output_image, crosscordinats = detect_objects(inputimg, output_image, vision_image,
                                                                 HsvFilter(0, 0, 0, 179, 255, 255, 0, 0, 0, 0), 100,
            ...
            midpoint, angle, output_image = ComputerVision.ImageProcessor.process_robot(inputimg, output_image,
                                                                                        bottom_left_corner=arenaCorners[
                                                                                            0], bottom_right_corner=
                                                                                        arenaCorners[1],
                                                                                        top_right_corner=arenaCorners[
                                                                                            2],
                                                                                        top_left_corner=arenaCorners[3])
            ...
            if (mode == "robot" and completion_flag.is_set()):
                if (angle is not None and midpoint is not None and ballcordinats):
                    correctmid = ComputerVision.ImageProcessor.convert_to_cartesian(midpoint, arenaCorners[0],
                                                                                    arenaCorners[1], arenaCorners[3],
                                                                                    arenaCorners[2])
                    '''
                    ...
                    await asyncio.create_task(
                        com.command_robot_async(correctmid, ballcordinats, angle, socket, completion_flag))
